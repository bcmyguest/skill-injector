...
pub fn rank_all_ctx(
    ...
    let mut hits: Vec<Hit> = index
        ...
        .collect();
    // `total_cmp`, not `partial_cmp(..).unwrap_or(Equal)`: with the latter a NaN
    // (should one slip past the channel guards) compares Equal to every score and
    // can land anywhere, including rank 0.
